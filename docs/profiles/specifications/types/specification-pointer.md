...
title: Specification Pointer
description: A specification whose authoritative text is owned by another repository,
  recorded here with the ownership it establishes.
generated:
...
Every rule below is the effective rule for a concept of type `Specification Pointer`:
the profile-wide rule and this type's own rule, already merged.

### Required

#### `authoritativeSpec` — required
