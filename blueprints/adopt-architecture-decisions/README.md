...
**v0.16.0** — so a new adopter needs no migration afterwards. Loading that descriptor requires
**`okf` 0.9.0.0 or later**.

A bundle already upgraded to v0.8.0 needs no edge to reach v0.16.0: no release since has changed
what the architecture-decision profile demands of a document. Repin the descriptor to v0.16.0,
