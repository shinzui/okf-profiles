...
current release, okf-profiles v0.18.0, which keeps that v0.2 contract and requires `okf` 0.9.0.0 or
later to load.
...
- `v0-2-migration-reference.md` is the authoritative per-profile change contract: what each profile
  ...
  diagnostic.
- `profile-pins.md` gives the v0.18.0 pinned-import line for each of the seven profile exports, to
...
If a bundle has a local descriptor pinning `okf-profiles`, move the pin to the v0.18.0 tag. Copy the
