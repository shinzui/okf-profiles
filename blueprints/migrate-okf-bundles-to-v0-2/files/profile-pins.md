# okf-profiles v0.18.0 pinned imports
...
v0.8.0 and is unchanged in v0.18.0, which additionally widens the profile schema with optional
`guidance`. Loading a v0.18.0 descriptor requires `okf` 0.9.0.0 or later.

## Freezing

> **The `sha256:` hashes below are deliberately absent.** They are added by
> `dhall freeze` against the real v0.18.0 tag; this file ships with the import line only so that
