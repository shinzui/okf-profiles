...
descriptors to the current release, **v0.17.0**.
...
- **`v0-2-migration-reference.md`** — the per-profile change table, the actor convention, the
  ...
  v0.8.0.
- **`profile-pins.md`** — the v0.17.0 pinned-import line for each of the seven profile exports, a
  descriptor template, the repinning diff, and the freezing rules. v0.17.0 adds no corpus
  requirement beyond v0.8.0; it widens the profile schema with optional `guidance`, which is why
  it needs a newer `okf`.

## Prerequisites

- **`okf` 0.9.0.0 or later** (required to load v0.17.0 descriptors) and **`dhall` 1.42 or later** in the target repository's environment.
